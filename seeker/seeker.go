...
func (s *Seeker) Run() {
	...
	if s.se == "google" {
		err = chromedp.Run(ctxt, s.searchGoogle())
	} else if s.se == "bing" {
		err = chromedp.Run(ctxt, s.searchBing())
	}
}
...
func (s *Seeker) searchBing() chromedp.Tasks {
	...
	return chromedp.Tasks{
		...
		chromedp.ActionFunc(func(c context.Context) error {
			var resCount string
			chromedp.Text(`.sb_count`, &resCount, chromedp.ByQuery).Do(c)
			...
			for i := 0; i <= s.maxPage; i++ {
				...
				if i != s.maxPage {
					chromedp.Click(`//*[@title="Next page"]`, chromedp.BySearch).Do(c)
				}
			}
			return nil
		}),
...
func (s *Seeker) searchGoogle() chromedp.Tasks {
	...
	return chromedp.Tasks{
		...
		chromedp.ActionFunc(func(c context.Context) error {
			var resCount string
			var xx string
			chromedp.Text(`#resultStats`, &resCount, chromedp.ByID).Do(c)
			...
			for i := 0; i <= s.maxPage; i++ {
				chromedp.Sleep(1 * time.Second).Do(c)
				chromedp.EvaluateAsDevTools(`
				...
				s.Results = append(s.Results, urls...)
				if i != s.maxPage {
					chromedp.Click(`#pnnext`, chromedp.NodeVisible).Do(c)
				}
			}
			return nil
		}),
